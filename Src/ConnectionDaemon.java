...
class ConnectionDaemon extends Thread{
	...
	public static void main(String args[]) throws Exception{
		...
		if (argCount>0)ConnectionDaemon.DROP_RATE=Double.parseDouble(args[0]);
		if (argCount>1)ConnectionDaemon.ERROR_RATE=Double.parseDouble(args[1]);
		//System.out.println(ConnectionDaemon.DROP_RATE + " " + ConnectionDaemon.ERROR_RATE);
	...
	public void loadTopology() throws Exception{
		...
		while((line=br.readLine())!=null){
			...
			if(tokens.length>1 && tokens[1].compareTo("CONNECTSTO")==0){
				...
				mappings.put(oneEnd,otherEnd); //can be done using single entry
	...
	synchronized public void registerConnection(String deviceId, Connection c){
		...
		if(o!=null) System.out.println(deviceId+" connected to "+otherEnd);
	}
	...
	//-------Function that may be used to add errors to frame to emulate noise in the medium
	byte[] addError(byte[] b){				
				...
				if (fep < ERROR_RATE) {											
					...
					int k=(int)(Math.random() * (b.length))+1; //value of k is random
					for (int i=0;i<k;i++) b[i]=(byte)(~b[(int)(Math.random() * (b.length + 1))]);
