...
class SimHost {
	...
	/**
	 * device id of the end deivce or router or switch at the other end of a
	 * connection
	 */
	String otherEnd;
	...
	HostConsole hConsole;
	
	String hostId="";
	String hostInfo="";

	/**
	 * reads configurations of end devices; and connections among end devices,
	 * routers and switches
	 * 
	 * @param deviceId
	 *            string representation of the device, Hn for end devices, Rn
	 *            for routers, Sn for switches; n being 1,2,3,...
	 */
	public void loadParameters(String deviceId) throws Exception {
		...
		while ((line = br.readLine()) != null) {
			...
			if (tokens[0].compareTo(deviceId) == 0) {
				...
				/*
				 * if(paramName.compareTo("CONNECTSTO")==0){ otherEnd=tokens[2];
				...
				if (paramName.compareTo("IPADDRESS") == 0) {
					...
					hostInfo+="Default Gateway: "+ gateway.getString()+"\n";
					...
					hostInfo+="MAC ADDRESS: "+ (int) ((byte) macAddress & 0xFF)+"\n";
				}
			}
		}
	}
	...
	/**
	 * reads in the ARP table from text file, loads in hashtable indexed by ip
	 * address having mac address as the content
	 */
	...
	public SimHost(String deviceId) {
		...
		new NlSend(this);
		new NlReceive(this);
		hConsole=new HostConsole(this, hostId);
	}
	 ...
	 * 
	 * @param ip
	 *            ip address encapsulated in an instance of IpAddress
	...
	int getMacFromArpTable(IpAddress ip) {
		...
		if (macAd != null)
			return Integer.parseInt(macAd);
		else
			return -1;
	}

	/**
	 * class NlSend takes input from command line in IP:MESSAGE format, converts
	 * the message into a packet and sends the packet to data link layer
	 */
	private class NlSend extends Thread {
		...
		public void run() {
			...
			JFrame simH=new JFrame();
			simH.setSize(300,200);
			simH.setTitle(hostId+" - Host");
			simH.setVisible(true);
			
			JTextArea infoArea=new JTextArea();
			
			
			simH.getContentPane().setLayout(null);
			
			JButton btnCon = new JButton("Console");
			btnCon.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					hConsole.setVisible(true);
				}
			});
			 
			infoArea.setBounds(1, 1, 298, 70);
			btnCon.setBounds(10, 100, 127, 23);
			simH.getContentPane().add(infoArea);
			infoArea.setText(hostInfo);
			simH.getContentPane().add(btnCon);
			...
			try {
				while (true) {
					...
					if (ipAddr.sameSubnet(dstIp, mask)) {
						...
						dstMac = getMacFromArpTable(gateway);

					if (dstMac < 0) {
						System.out
								.println("MAC Address of Destination not found");
						continue;
					}
	...
	/**
	 * class NlReceive receives a packet by converting a frame received from
	 * data link layer
	 */
	private class NlReceive extends Thread {
		...
		public void run() {
			while (true) {
				...
				if (p.getDstIp().sameIp(ipAddr))
				{
					System.out.println("\tShowing Packet: " + p.getString()
							+ "\n");
					hConsole.consoleOutputTextArea.append("\tShowing Packet: " + p.getString()
							+ "\n");
				}
			}
		}
